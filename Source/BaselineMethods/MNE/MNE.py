...
def train_deepwalk_embedding(walks, iteration=None, size=128):
    if iteration is None:
        iteration = 100
    model = Word2Vec(walks, size, window=5, min_count=0, sg=1, workers=4, iter=iteration)
    return model
...
def train_embedding(current_embedding, walks, layer_id, iter=10, info_size=10, base_weight=1):
    ...
    new_model = MNE(training_data, size=128, window=5, min_count=0, sg=1, workers=4, iter=iter, small_size=info_size, initial_embedding=initial_embedding, base_weight=base_weight)
...
def train_model(network_data):
    ...
    base_G = Random_walk.RWGraph(get_G_from_edges(base_network), 'directed', 2, 1)
    print('finish building the graph')
    base_G.preprocess_transition_probs()
    base_walks = base_G.simulate_walks(15, 10)
    base_embedding, _, _, index2word = train_embedding(None, base_walks, 'Base')
    final_model = dict()
    ...
    for layer_id in network_data:
        ...
        layer_G = Random_walk.RWGraph(get_G_from_edges(tmp_data), 'directed', 2, 1)
        layer_G.preprocess_transition_probs()
        layer_walks = layer_G.simulate_walks(20, 20)
