...
class Mergeing_vec_N2V:
    ...
    def run(self):
        ...
        # 实验：多层网络的表示学习方法(MHRW, FFS)
        MHWL = [] # 跳转MHRW采样-避免随机游走对度大的节点偏执
        # FFSN = [] # 林火采样
        ...
        ExpansionSample_MHRW = partial(MHRW.random_walk_sampler, nx_graphs_sampled, node_exit, self.walk_length, nodeinfluence, True)
        ...
        with ThreadPool(processes=4) as pool:
            for walks in range(self.num_walks):
                MHWL.extend(pool.map(ExpansionSample_MHRW, allnodes))
